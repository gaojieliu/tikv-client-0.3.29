...
use crate::proto::kvrpcpb::{self, RawScanRequest, RawScanResponse, RegionKeys};
use crate::proto::metapb;
use crate::raw::{RawRpcRequest, lowering::*};
...
impl<PdC: PdClient> Client<PdC> {
    ...
    pub async fn batch_get_optimized_new(
        ...
        let request = new_raw_batch_get_optimized_request(keys.into_iter(), self.cf.clone());
        let plan = crate::request::PlanBuilder::new(self.rpc.clone(), self.keyspace, request)
    ...
    pub async fn batch_get_optimized(
        ...
        // Step 2: Map keys to regions and then group by store ID
        ...
        // Step 3: Send one batch_get request per store in parallel
        let mut tasks = vec![];

        for (store_id, region_keys_map) in store_region_keys_map {
            let cf = self.cf.clone();

            // Compose raw batch get optimized request for this store
            let mut req = kvrpcpb::RawBatchGetOptimizedRequest::default();
            req.regions = region_keys_map.into_values().collect();
            req.maybe_set_cf(cf);
            let ctx = req.context.get_or_insert(kvrpcpb::Context::default());
            ctx.api_version = self.keyspace.api_version().into();
            // Get store for this store_id
            let store = store_client_map.get(&store_id).unwrap().clone();
            ...
            let task = tokio::spawn(async move {
                ...
                let response = *response_box
                    .downcast::<crate::proto::kvrpcpb::RawBatchGetResponse>()
                    .map_err(|_| Error::StringError("Failed to downcast response".to_string()))?;
                
                
                // Check for region errors
                if let Some(err) = response.region_error {
                    return Err(Error::RegionError(Box::new(err)));
                }
                
                // Convert response to KvPairs
        ...
        // Step 4: Collect results from all stores
        let mut all_pairs = Vec::new();
        
        for task in tasks {
            let pairs = task.await.map_err(|e| Error::StringError(e.to_string()))??;
            all_pairs.extend(pairs);
        }
        
        // Step 5: Truncate keyspace from results
        let result: Vec<_> = all_pairs
            .into_iter()
            .map(|pair| pair.truncate_keyspace(self.keyspace))
            .collect();
    ...
    /// Create a new *atomic* 'compare and set' request with ttl.
    ///
    /// Once resolved this request will result in an atomic `compare and set`
    /// operation for the given key with ttl.
    ///
    /// If the value retrieved is equal to `current_value`, `new_value` is
    /// written with the specified ttl.
    ///
    /// # Return Value
    ///
    /// A tuple is returned if successful: the previous value and whether the
    ...
    pub async fn compare_and_swap_with_ttl(
        ...
        debug!("invoking raw compare_and_swap_with_ttl request");
        self.assert_atomic()?;
        let key = key.into().encode_keyspace(self.keyspace, KeyMode::Raw);
    ...
    async fn scan_inner(
        ...
        while current_limit > 0 {
            ...
            let res = if !reverse {
                self.retryable_scan(scan_args).await?
            } else {
                self.retryable_reverse_scan(scan_args).await?
            };

            let scan_response = res.response;
            let next_key = res.next_key;

            let mut kvs = scan_response
                .map(|r| r.kvs.into_iter().map(Into::into).collect::<Vec<KvPair>>())
                .unwrap_or(Vec::new());

            if !kvs.is_empty() {
                current_limit -= cmp::min(kvs.len() as u32, current_limit);
            ...
            //Create the next range for the scan
            if reverse {
                if !start_key.is_empty() && start_key >= next_key {
                    break;
                } else {
                    range = BoundRange::new(range.from, Bound::Included(next_key));
                }
            } else if end_key
                .clone()
                .is_some_and(|ek| !ek.is_empty() && ek <= next_key)
            {
                break;
            } else {
                range = BoundRange::new(Bound::Included(next_key), range.to);
            }
        }
    ...
    async fn retryable_scan(&self, mut scan_args: ScanInnerArgs) -> Result<ScanInnerResponse> {
        let (start_key, _end_key) = scan_args.range.clone().into_keys();
        loop {
            ...
            let res = self
                .handle_region_scan(request, region, |r: RegionWithLeader| r.end_key(), store)
                .await;

            if let Some(value) = Self::process_scan_response(&mut scan_args, res).await {
                return value;
            }
        }
    }

    async fn retryable_reverse_scan(
        &self,
        mut scan_args: ScanInnerArgs,
    ) -> Result<ScanInnerResponse> {
        if let (start_key, Some(end_key)) = scan_args.range.clone().into_keys() {
            let mut end_key = end_key;
            let mut search_prev_region = false;
            loop {
                let region: RegionWithLeader = if search_prev_region {
                    if end_key.zero_terminated() {
                        end_key.0.pop().unwrap();
                    }
                    self.rpc
                        .clone()
                        .prev_region_for_key(&end_key.clone())
                        .await?
                } else {
                    self.rpc.clone().region_for_key(&end_key.clone()).await?
                };
                if !search_prev_region
                    && region.start_key().clone().next_key() == end_key.clone()
                    && !region.start_key().is_empty()
                {
                    search_prev_region = true;
                    end_key = region.start_key();
                    continue;
                }
                let range = (start_key.clone(), end_key.clone()).into();
                let store = self.rpc.clone().store_for_id(region.id()).await?;
                let request = new_raw_scan_request(
                    range,
                    scan_args.limit,
                ...
                let res = self
                    .handle_region_scan(request, region, |r: RegionWithLeader| r.start_key(), store)
                    .await;
            ...
            Err(StringError("no end key found for reverse scan".to_string()))
        }
    }

    async fn process_scan_response(
        scan_args: &mut ScanInnerArgs,
        res: Result<ScanInnerResponse>,
    ) -> Option<Result<ScanInnerResponse>> {
        match res {
            Ok(r) => return Some(Ok(r)),
            Err(Error::BackoffAndRetryForStoreError(err)) => {
                return if let Some(duration) = scan_args.backoff.next_delay_duration() {
                    sleep(duration).await;
                    None
                } else {
                    Some(Err(RegionError(err)))
                }
            }
            Err(Error::RetryableStoreError(_)) => {}
    ...
    async fn handle_region_scan<F>(
        &self,
        raw_scan_request: RawScanRequest,
        region: RegionWithLeader,
        next_key_fn: F,
        store: RegionStore,
    ) -> Result<ScanInnerResponse>
    where
        F: Fn(RegionWithLeader) -> Key,
    {
        let resp = self
            .do_store_scan(store.clone(), raw_scan_request.clone())
            .await;
        match resp {
            Ok(mut r) => {
                if let Some(err) = r.region_error() {
                    let status =
                        plan::handle_region_error(self.rpc.clone(), err.clone(), store.clone())
                            .await?;
                    return if status {
                        Err(Error::RetryableStoreError(Box::new(err)))
                    } else {
                        Err(Error::BackoffAndRetryForStoreError(Box::new(err)))
                    };
                }
                let r = ScanInnerResponse {
                    response: Some(r),
                    next_key: next_key_fn(region),
                };
                Ok(r)
            }
            Err(err) => Err(err),
...
mod tests {
    ...
    use crate::mock::{InjectableMockPdClient, MockKvClient, MockRegion};
    ...
    #[tokio::test]
    async fn test_raw_forward_scan() -> Result<()> {
        let mut fake_region1 = MockRegion::default();
        fake_region1.id(1);
        fake_region1.start_key(vec![]);
        fake_region1.end_key("k4");
        fake_region1.entry("k1", "v1");
        fake_region1.entry("k2", "v2");
        fake_region1.entry("k3", "v3");

        let mut fake_region2 = MockRegion::default();
        fake_region2.id(2);
        fake_region2.start_key("k4");
        fake_region2.end_key("k7");
        fake_region2.entry("k4", "v4");
        fake_region2.entry("k5", "v5");
        fake_region2.entry("k6", "v6");

        let mut fake_region3 = MockRegion::default();
        fake_region3.id(3);
        fake_region3.start_key("k7");
        fake_region3.end_key("k99");
        fake_region3.entry("k7", "v7");
        fake_region3.entry("k8", "v8");
        fake_region3.entry("k9", "v9");

        let mut fake_region4 = MockRegion::default();
        fake_region4.id(4);
        fake_region4.start_key("k99");
        fake_region4.end_key(vec![]);
        fake_region4.entry("k99", "v99");
        fake_region4.entry("k998", "v998");
        fake_region4.entry("k9988", "v9988");

        let mut pd_client = InjectableMockPdClient::default();
        pd_client.region(Arc::new(fake_region1));
        pd_client.region(Arc::new(fake_region2));
        pd_client.region(Arc::new(fake_region3));
        pd_client.region(Arc::new(fake_region4));

        let client = Client {
            rpc: Arc::new(pd_client),
            cf: Some(ColumnFamily::Default),
            backoff: DEFAULT_REGION_BACKOFF,
            atomic: false,
        ...
        let range = "k1"..="k8";
        let range = BoundRange::from(range);
        let resps = client.scan(range, 100).await?;
        let resps: Vec<_> = resps
            .into_iter()
            .map(|kv_pair: KvPair| {
                (
                    String::from_utf8(kv_pair.0 .0).unwrap(),
                    String::from_utf8(kv_pair.1).unwrap(),
                )
            })
            .collect();
        assert_eq!(
        ...
        let range = BoundRange::from(range);
        let resps = client.scan(range, 100).await?;
        let resps: Vec<_> = render_scan_response(resps);
        assert_eq!(
            resps,
            vec![
                ("k1", "v1"),
        ...
        let range = ..;
        let range = BoundRange::from(range);
        let resps = client.scan(range, 4).await?;
        let resps: Vec<_> = render_scan_response(resps);
        assert_eq!(
            resps,
            vec![("k1", "v1"), ("k2", "v2"), ("k3", "v3"), ("k4", "v4")]
                .into_iter()
                .map(|(k, v)| (k.to_string(), v.to_string()))
                .collect::<Vec<_>>()
        );
        let range = ..;
        let range = BoundRange::from(range);
        let resps = client.scan(range, 0).await?;
        let resps: Vec<_> = render_scan_response(resps);
        assert_eq!(resps, vec![]);

        let range = ..;
        let range = BoundRange::from(range);
        let resps = client.scan(range, 7000000).await;
        assert!(resps.is_err());
    ...
    #[tokio::test]
    async fn test_raw_reverse_scan() -> Result<()> {
        //Add start_key and end_key to the MockRegion
        let mut fake_region1 = MockRegion::default();
        fake_region1.id(1);
        fake_region1.start_key("k1");
        fake_region1.end_key("k4");
        fake_region1.entry("k1", "v1");
        fake_region1.entry("k2", "v2");
        fake_region1.entry("k3", "v3");
        ...
        let resps = client.scan_reverse(range, 6).await?;
        let resps: Vec<_> = render_scan_response(resps);
        assert_eq!(
            resps,
            vec![
    ...
    fn render_scan_response(resps: Vec<KvPair>) -> Vec<(String, String)> {
        resps
            .into_iter()
            .map(|kv_pair: KvPair| {
                (
