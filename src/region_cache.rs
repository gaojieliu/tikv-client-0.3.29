...
impl<C: RetryClientTrait> RegionCache<C> {
    ...
    // Retrieve cache entry by key. If there's no entry, query PD and update cache.
    pub async fn get_prev_region_by_key(&self, key: &Key) -> Result<RegionWithLeader> {
        let region_cache_guard = self.region_cache.read().await;
        let res = {
            region_cache_guard
                .key_to_ver_id
    ...
    /// Force read through (query from PD) for prev region
    ...
    impl RetryClientTrait for MockRetryClient {
        ...
        async fn get_prev_region(self: Arc<Self>, key: Vec<u8>) -> Result<RegionWithLeader> {
            ...
            self.regions
                ...
                .await
                .iter()
                .take_while(|(_, r)| !r.contains(&key.clone().into()))
                .map(|(_, r)| r.clone())
                .last()
                .ok_or_else(|| Error::StringError("MockRetryClient: region not found".to_owned()))
        }
