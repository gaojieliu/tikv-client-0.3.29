...
use crate::proto::pdpb::GetRegionRequest;
use crate::Error;
use crate::Result;
...
impl Cluster {
    ...
    //This is primarily being used for reverse scan.
...
pub(crate) struct PdPbGetPrevRegionRequest(GetRegionRequest);

impl IntoRequest<GetRegionRequest> for Request<PdPbGetPrevRegionRequest> {
    fn into_request(self) -> Request<GetRegionRequest> {
        let req = self.into_inner().0;
        Request::new(req)
    }
}
...
#[async_trait]
impl PdMessage for PdPbGetPrevRegionRequest {
    type Client = pdpb::pd_client::PdClient<Channel>;
    type Response = pdpb::GetRegionResponse;

    async fn rpc(req: Request<Self>, client: &mut Self::Client) -> GrpcResult<Self::Response> {
        Ok(client.get_prev_region(req).await?.into_inner())
    }
}
